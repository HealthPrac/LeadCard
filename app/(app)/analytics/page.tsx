...
export default async function AnalyticsPage() {
  ...
  const { count: total } = await supabase.from('leads').select('*', { count: 'exact', head: true }).eq('subscriber_id', subscriber.id)

  // Last 7 days
  const since7 = new Date(Date.now() - 7 * 86400000).toISOString()
  const { count: last7 } = await supabase.from('leads').select('*', { count: 'exact', head: true }).eq('subscriber_id', subscriber.id).gte('created_at', since7)
  ...
  return (
    <div style={{ maxWidth: 900 }}>
      ...
      <p style={{ fontSize: 13, color: 'var(--muted)', margin: '0 0 28px' }}>Lead capture overview. Detailed view analytics coming in a future update.</p>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 28 }}>
        {statBox('All-time leads', total ?? 0)}
        {statBox('Last 7 days', last7 ?? 0)}
        {statBox('Sources', sortedSources.length || '—')}
      </div>
              ...
              return (
                <div key={source} style={{ padding: '10px 22px', display: 'flex', alignItems: 'center', gap: 16 }}>
                  ...
                  <div style={{ flex: 1, height: 6, background: 'var(--cream-2)', borderRadius: 99, overflow: 'hidden' }}>
                    <div style={{ width: `${pct}%`, height: '100%', background: 'var(--sage)', borderRadius: 99, transition: 'width 0.4s' }} />
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--muted)', width: 60, textAlign: 'right', flexShrink: 0 }}>{count} ({pct}%)</div>
                </div>
              )
            })}
          </div>
      ...
      <div style={{ background: 'var(--sage-tint)', borderRadius: 14, padding: '28px 24px', display: 'flex', alignItems: 'center', gap: 18 }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--muted)', margin: 0 }}>Card views, tap-to-scan conversion, and heatmaps will appear here once available.</p>
        </div>
