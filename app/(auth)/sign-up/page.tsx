  ...
  if (checkInbox) {
    return (
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 14, color: 'rgba(23,23,28,0.52)', margin: '0 0 28px', lineHeight: 1.6 }}>
          Click the link in the email to verify your address and activate your account.
          It only takes a second.
        </p>
        <div style={{ padding: '14px 18px', background: '#ffffff', borderRadius: 10, border: '1px solid rgba(23,23,28,0.10)', fontSize: 13, color: 'rgba(23,23,28,0.52)', marginBottom: 24 }}>
          Didn&apos;t get it? Check your spam folder or{' '}
          <button
            onClick={() => setCheckInbox(false)}
            style={{ background: 'none', border: 'none', padding: 0, color: '#17171C', textDecoration: 'underline', cursor: 'pointer', fontSize: 13, fontFamily: 'inherit' }}
          >
            try a different email
          </button>
          .
        </div>
        <Link href="/sign-in" style={{ fontSize: 13, color: 'rgba(23,23,28,0.52)', textDecoration: 'underline' }}>
          Back to sign in
        </Link>
      </div>
    )
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div>
          <label style={labelStyle}>Password <span style={{ color: 'rgba(23,23,28,0.52)', fontWeight: 400 }}>(8+ characters)</span></label>
      ...
      <p style={{ textAlign: 'center', fontSize: 13, color: 'rgba(23,23,28,0.52)', marginTop: 20 }}>
        ...
        <Link href="/sign-in" style={{ color: '#17171C', textDecoration: 'underline' }}>Sign in</Link>
      </p>
    </div>
  )
}
